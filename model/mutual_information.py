...
def conditional_mutual_information(prediction, model_specs):
    ...
    """
    model_specs_copy = {}
    # Assure that in the model specs we always have the word 'model' in the key
    ...
    # Reference contains, for each model with the name 'model_#', the generalization gap
    reference = {}
    for mid in model_specs:
    ...
    # get names of the hyperparameters
    all_mid = list(model_specs.keys()) # Contains the names of the models
    hp_names = list(model_specs[all_mid[0]]['hparams'].keys()) # Contains the names of the hyperparameters
    hp_values = {k: model_specs[all_mid[0]]['hparams'][k]['possible_values'] for k in hp_names} # Possible values for each hyperparameter
    ...
    for h1 in hp_names:
        for h2 in hp_names:
            if h1 == h2:
                continue
            # Here we have different hyperparameters h1 and h2
