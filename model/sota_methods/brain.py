...
def knn_over_matrix(matrix, k):
    ...
    thresholds = matrix[np.arange(matrix.shape[0]), temp].reshape(-1,
                                                                  1)  # Transform matrix into a column matrix of maximums
...
def complexity(model, dataset):
    ...
    for a in range(number_of_classes):
        datasets.append(dataset.filter(lambda data, labels: tf.equal(labels, a)).repeat().shuffle(100).batch(
            examples_per_class).__iter__())
